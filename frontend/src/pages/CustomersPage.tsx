...
export const CustomersPage: React.FC = () => {
  ...
  const [customerToDelete, setCustomerToDelete] = useState<CustomerDirectoryItem | null>(null);
  ...
  const handleCreateCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBannerMessage({
        type: 'success',
        text: `Customer account "${newName.trim()}" created successfully.`,
      });
      setNewName('');
      setNewEmail('');
      setNewTier('BRONZE');
      ...
      setBannerMessage({
        type: 'error',
        text: err.response?.data?.error?.message || 'Failed to create customer',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDeleteCustomer = async () => {
    ...
    try {
      ...
      setBannerMessage({
        type: 'success',
        text: `Customer account "${customerToDelete.name}" and associated records deleted successfully.`,
      });
      ...
      setBannerMessage({
        type: 'error',
        text: err.response?.data?.error?.message || 'Failed to delete customer account.',
      });
    } finally {
      setDeleting(false);
    }
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          className="text-blue-600 hover:text-blue-800 text-xs font-semibold hover:bg-blue-50 px-2.5 py-1.5 rounded-lg transition inline-flex items-center space-x-1"
                        ...
                        <button
                          ...
                          className="text-rose-600 hover:text-rose-800 text-xs font-semibold hover:bg-rose-50 px-2.5 py-1.5 rounded-lg transition inline-flex items-center space-x-1"
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full p-6 animate-in fade-in zoom-in-95">
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
              <button
                ...
                className="px-3.5 py-2 text-xs font-bold text-rose-600 hover:text-rose-800 hover:bg-rose-50 rounded-xl transition flex items-center space-x-1.5"
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 animate-in fade-in zoom-in-95">
            <div className="flex items-start space-x-3.5">
              ...
              <div>
                ...
                <p className="text-xs font-mono text-slate-400 mt-0.5">{customerToDelete.email}</p>
              </div>
            </div>

            <div className="mt-4 p-3.5 bg-rose-50/90 border border-rose-200 rounded-xl text-xs text-rose-900 space-y-1.5">
              <div className="font-bold flex items-center space-x-1 text-rose-950">
                <ShieldAlert className="w-3.5 h-3.5 text-rose-600" />
                <span>Irreversible Account Deletion</span>
              </div>
              <p className="text-[11px] text-rose-800 leading-relaxed">
                This customer currently has{' '}
                <span className="font-bold">{customerToDelete.totalQuotes} quotation(s)</span> and{' '}
                <span className="font-bold">{customerToDelete.confirmedOrders} confirmed deal(s)</span>.
                Deleting this customer will permanently delete all associated quotes, delivery splits, and governance history.
              </p>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end space-x-3">
              ...
              <button
                ...
                className="px-5 py-2 text-xs font-bold bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-sm transition disabled:opacity-50 flex items-center space-x-1.5"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{deleting ? 'Deleting...' : 'Yes, Delete Customer'}</span>
              </button>
            </div>
          </div>
        </div>
